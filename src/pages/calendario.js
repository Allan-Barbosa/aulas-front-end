...
export default function Turmas() {
  ...
  const [minhasAulasvagas, setMinhasAulasVagas] = useState(null);
  useEffect(() => {
 ...
 async function buscarAulasVagas() {
    try {
      console.log('dentro')
      const resposta = await fetch('https://api-aulas.onrender.com/api/classroom/my_dates/' + turmaCode, {
        method: 'GET',
        headers: {
...
const colors2 = [
  ...
  return (
    <VStack config={config} safeArea flex={1}>
      ...
      {minhasAulasvagas.map((aulavaga, index) => (
            (
            <Turma
              key={index}
              title={turmaName}
